...
运营管理端新增综合查询中心，支持 `ADMIN`、`RISK_OFFICER`、`OPERATOR` 通过 `/api/search` 按关键词检索订单、电子票、支付、退款、常用乘车人、风险事件、Outbox 事件、站内通知和操作日志。普通乘客 `USER` 无权访问该接口。搜索结果中关联订单的记录会复用现有订单详情弹窗，并对证件号、手机号等敏感字段进行脱敏展示。
