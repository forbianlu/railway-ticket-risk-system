...
乘客端接口统一为 `/api/passenger/**`。这些接口从 JWT 中读取当前 `userId`，乘客不能通过请求参数覆盖用户 ID，也不能支付、关闭或退票其他乘客的订单。

常用乘车人接口同样绑定当前 `USER`。乘客只能管理自己的 `passenger_profiles`，不能使用其他乘客的 `travelerId` 创建订单。接口响应只返回脱敏证件号和脱敏手机号，订单与电子票也只对外展示脱敏快照。

`USER` 不允许访问管理端接口，包括订单管理、支付退款管理、风险管理、审计日志、Outbox 事件、缓存限流和运营看板等路径。拦截器会对已登录的 `USER` 访问这些管理路径返回 403，管理端原有角色权限保持不变。
