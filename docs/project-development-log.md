...
- 新增乘客电子票分页接口：`GET /api/passenger/tickets`，支持按 `ISSUED`、`REFUNDED`、`CANCELLED` 状态筛选。
- 电子票查询始终使用当前 USER 的 `userId`，不接受前端传入用户归属，避免跨乘客查看票面。
- 乘客端新增“我的电子票”区域，使用票面卡片展示车次、站点、日期、席别、票价、出票时间和脱敏乘车人信息。
- 订单详情弹窗新增订单进度时间线，串联下单、支付、出票、退票和退款结果。
- 电子票详情从普通字段列表升级为行程单样式，显示票号、路线、乘车时间、实名快照和票面状态。
- 集成测试补充乘客电子票夹权限边界：当前乘客能查到自己的电子票，其他乘客不能查到。
- Maven 测试通过：`Tests run: 42, Failures: 0, Errors: 0, Skipped: 0`。
- 前端脚本检查通过：`frontend/app.js`、`frontend/passenger.js`。
## 入口页视觉重设计
